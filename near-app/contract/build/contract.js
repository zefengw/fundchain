...
function simulateCoinFlip() {
  ...
  const randomString = randomSeed();

  // If the last charCode is even we choose heads, otherwise tails
  return randomString.charCodeAt(0) % 2 ? 'heads' : 'tails';
}
// 
let CoinFlip = (_dec = NearBindgen({}), _dec2 = call({}), _dec3 = view(), _dec(_class = (_class2 = class CoinFlip {
  points = new UnorderedMap("points");
  ...
  */
  flip_coin({
    ...
    log(`${player} chose ${player_guess}`);

    // Simulate a Coin Flip
    const outcome = simulateCoinFlip();

    // Get the current player points
    let player_points = this.points.get(player, {
      defaultValue: 0
    });

    // Check if their guess was right and modify the points accordingly
    if (player_guess == outcome) {
      log(`The result was ${outcome}, you get a point!`);
      player_points += 1;
    } else {
      log(`The result was ${outcome}, you lost a point`);
...
}, (_applyDecoratedDescriptor(_class2.prototype, "flip_coin", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "flip_coin"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "points_of", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "points_of"), _class2.prototype)), _class2)) || _class);
function points_of() {
  const _state = CoinFlip._getState();
  if (!_state && CoinFlip._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = CoinFlip._create();
  if (_state) {
    CoinFlip._reconstruct(_contract, _state);
  }
  const _args = CoinFlip._getArgs();
  const _result = _contract.points_of(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(CoinFlip._serialize(_result, true));
}
function flip_coin() {
  const _state = CoinFlip._getState();
  if (!_state && CoinFlip._requireInit()) {
  ...
  const _args = CoinFlip._getArgs();
  const _result = _contract.flip_coin(_args);
  CoinFlip._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(CoinFlip._serialize(_result, true));
}

export { flip_coin, points_of };
